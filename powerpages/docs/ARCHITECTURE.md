...
#### Base Case Budget Overlay

When the user loads the Base Case Budget, the dashboard derives each category's monthly growth rate from the mapped budget balances, not from GL balances:

```
budgetGrowthRate(category, month) =
  (budgetBalance(category, month) - budgetBalance(category, priorMonth))
  / abs(budgetBalance(category, priorMonth))
```

The resulting percentage is then applied to the liquidity model's rolling balance, which begins from the most recent actual GL balance. The budget balance is therefore the source of the **change rate**; it does not replace the actual starting position. This intentionally preserves the actual balance-sheet position while using the budget to forecast the direction and magnitude of subsequent balance movements.

Accordingly, a modeled forecast balance is not expected to equal the corresponding absolute budget balance when the actual starting balance differs from the budget's opening balance. The expected relationship is that the model reflects the budget-derived period-over-period percentage movement on top of the actual starting balance.
...
## 24. Liquidity Scenario Library Contract

Liquidity scenarios are separate from hierarchy metadata. The shared audit artifact is
`powerpages/Settings/Liquidity Scenario Library.xlsx`; it must never be merged into
`Dashboard Hierarchies.xlsx`.

Each scenario revision records its scenario ID, name, revision, favorite/current
status, source as-of date, model version, author, UTC timestamp, and serialized
assumptions. A scenario snapshot includes growth rates, no-new-business behavior,
rate overrides, liquidity-source haircuts and floors, dividends, and scenario metadata.
Loading a scenario with a different source date or model version requires an explicit
confirmation.

The current self-contained Liquidity Dashboard can safely exchange scenario bundles
through explicit JSON import/export and keep a local working copy. A future shared
folder roll-forward implementation must use a workbook-aware persistence layer to
write the scenario library and the `Liquidity` mapping tab without reconstructing or
damaging existing hierarchy sheets. Browser cache is not an audit store.
